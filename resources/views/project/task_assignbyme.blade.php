  ...
  <div class="main-panel">
    <div class="dashboard-bodypart">
      <div class="dashboard-bodypart-in">
        <h1> Task <span> <i class="fa fa-tasks"></i> </span> <small>Task Assigned By Me </small> </h1>
        <div class="card">
          <div class="card-body">
                ...
                <div id="no-more-tables">
                  <table class="table table-striped">
                    ...
                    @foreach( $tasks as $task)
                      <tr>
                        <td class="py-1"> <span>{{ $task->projectname }}  </span></td>
                        <td> {{ $task->task_name }} </td>
                        <td> {{ $task->name }} </td>
                        <td> {{ \Carbon\Carbon::parse($task->task_assing_on)->format('d/m/Y') }}  </td>
                        <td> {{ \Carbon\Carbon::parse($task->task_duedate)->format('d/m/Y') }} </td>
                        <td>@if( $task->status == 'Assigned' )
                        		<span class="alert alert-danger" id="statusalert{{ $task->project_settings_id }}">
                        			<span id="span{{ $task->project_settings_id }}"><input type="checkbox" name="status" class="status" data-project-settings-id="{{ $task->project_settings_id }}"> Incomplete </span>
                        		</span>

                        	@elseif( $task->status == 'Completed' )
                        		<span class="alert alert-success" id="statusalert{{ $task->project_settings_id }}">
                        			<span id="span{{ $task->project_settings_id }}"><input type="checkbox" name="status" class="status" data-project-settings-id="{{ $task->project_settings_id }}" checked> Complete</span>
                        		</span>
                        	@endif


                        </td>
                        <td> <span id="completedate{{ $task->project_settings_id }}">{{ \Carbon\Carbon::parse( $task->completed_on )->format('d/m/Y') }}</span> </td>
