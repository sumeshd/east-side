...
<div class="main-panel">
    <div class="dashboard-bodypart">
        <div class="dashboard-bodypart-in">
            <h1> Project Settings <span> <i class="fa fa-cog"></i> </span> <small>Projact Settings - Execution </small> </h1>
            <div class="card">
                <div class="card-body">

                    <h2>  <a href="{{ url('projectVS/'.$project->id) }}" class="combtnDiv"> Project View Settings  </a> </h2>
                    <div class="boxthree">
                        @php $settings_name = explode(",", $project->settings_name ); @endphp
                      ...
                      <ul>
                        @if( !empty($presales))
                        <li> <a href="{{ url('projectsettings/'.$presales.'/'.$project->id) }}">Project Presales </a> </li>
                        @endif
                        @if(!empty($postsales))
                        <li> <a href="{{ url('projectsettings/'.$postsales.'/'.$project->id) }}">Project Postsales </a> </li>
                        @endif
                        @if(!empty($execution))
                        <li  class="active"> <a href="{{ url('projectsettings/'.$execution.'/'.$project->id) }}">Project Execution </a> </li>
                        @endif
                      </ul>
                    </div>
                    ...
                    <div class="progressDiv">
                        <div class="row">
                            <div class="col-md-8">
                                ...
                                <hr />
                                <form action="{{ url('project/executionstore') }}" >
                                    @csrf
...
<script type="text/javascript">
    ...
    $("#add-item").submit(function (e) {
        ...
        if(view == 0 ){
            ...
            var view_tf = "<a href='#' class='btn btn-success'> <img src={{ url('assets/images/icon/view-svgrepo-com.svg') }} alt='icon' style='width:17px;'></a>";
        }

        if(upload == 0 ){
            var upload_tf = "";
        }else{
            var upload_tf = "<a href='#' class='btn btn-success'> <img src={{ url('assets/images/icon/upload-svgrepo-com.svg') }} alt='icon' style='width:17px;'></a>";
        }

        if(download == 0 ){
            var download_tf = "";
        }else{
            var download_tf = "<a href='#' class='btn btn-success'> <img src={{ url('assets/images/icon/download-svgrepo-com.svg') }} alt='icon' style='width:17px;'></a>";
        }

        if(comments == 0 ){
            var comments_tf = "";
        }else{
            var comments_tf = "<a href='#' class='btn btn-success'> <img src={{ url('assets/images/icon/comment-svgrepo-com.svg') }} alt='icon' style='width:17px;'></a>";
        }
